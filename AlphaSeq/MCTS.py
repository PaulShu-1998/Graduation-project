...
class Node():
    def __init__(self, selfPlay, cumulativeMove, num_Moves, n_actions, evaluating_fn, calc_reward, fromlink, parent=None):
        """
        Initial function
        :param selfPlay:the binary variable represent whether only playing MCTS or using DNN
        :param cumulativeMove:the immediate codebook i.e. immediate matrix
        :param num_Moves: totally number of time steps i.e. N*K/l
        :param n_actions: number of actions for the state
        :param evaluating_fn:
        :param calc_reward: calculate Value Function V(s_i)
        :param fromlink: previous action which lead to this state
        :param parent: Parent node
        Prior_sa: the distribution of probabilities
        value: estimated Reward
        """
        # a node has a state (of length num_Moves), a parent
        self.selfPlay = selfPlay
        self.move = cumulativeMove
        ...
        self.N_sa = np.zeros(self.n_actions)  # N(s,a) visited number
        self.W_sa = np.zeros(self.n_actions)  # W(s,a) Totally reward got
        self.Q_sa = np.zeros(self.n_actions)  # Q(s,a) mean value
    ...
    def terminalNode(self):
        if len(self.move) == self.num_Moves:  # the depth of the tree <= num_Moves
            return True
        return False

    def add_child(self, cumulativeMove, fromlink):
        child = Node(self.selfPlay, cumulativeMove, self.num_Moves, self.n_actions, self.evaluating_fn, self.calc_reward, fromlink, self)
...
def bestMove(node):
    """
    To find out which child node to move
    :param node:current state
    :return: the index of the next state from child list
    """
...
def cal_piVec(N_sa, tau=0):
    ...
    """
    if tau == 1:
        prob = N_sa/np.sum(N_sa)
    elif tau == 0:
        prob = np.zeros(len(N_sa))
        prob[np.argmax(N_sa)] = 1
    else:
        prob = softmax(1.0/tau * np.log(N_sa))
...
def TreePolicy(node, VisitedState, stepSize, flag):
    ...
    while node.terminalNode() == False:
        ...
        if nextmoveIndex in node.seenChildIndex:
            ...
            realNextMoves = np.array([(nextmoveIndex >> k) & 1 for k in range(0, stepSize)])[::-1]  # 按二进制的最后一位一一比较，最后一位是1的就变成数组里第一个元素也为1，最后反转数组
            cumulativeMove = np.append(node.move, 2*realNextMoves-1)  # 把0变成-1
            node.add_child(cumulativeMove, fromlink=nextmoveIndex)

            if flag == 1:
                VisitedState.store(cumulativeMove)

            return node.children[-1]  # return the newly-added node
...
def MCTS_main(args, VisitedState, stepSize, n_steps, evaluating_fn, calc_reward, selfPlay):
    """
    run MCTC simulation
    :param args: basic params
    :param VisitedState: visited tree
    :param stepSize: l in paper
    :param n_steps: the depth of the tree, i.e. N*K/l
    :param evaluating_fn:
    :param calc_reward:
    :param selfPlay: the binary variable represent whether only playing MCTS or using DNN
    piVec: 当前节点的转移概率
    ...
    while eachstep < n_steps:
        ...
        if eachstep == 0:
            root = Node(selfPlay, cumulativeMove, args.N*args.K, args.Q**stepSize, evaluating_fn, calc_reward, None, parent=None)
        ...
        piVec = cal_piVec(root.N_sa, tau)  # current vertex transformation probabilities

        # temporarily store
        currentState = np.append(cumulativeMove, np.zeros(args.N * args.K - len(cumulativeMove)))
        if selfPlay == 1:
        ...
        # root.parent = None
        cumulativeMove = root.move

        eachstep += 1

    return cumulativeMove, temp_store
